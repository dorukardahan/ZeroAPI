...
These examples are deterministic fresh-install artifacts generated by `npm run examples:refresh` from the 2026-09-06 snapshot. They use GPT-5.6 Sol/Terra/Luna, GLM-5.3/Flash, Kimi Coding K3-256k, MiniMax-M3/M2.7, and Grok 4.6/4.5/Build 0.1/4.3 where selected. Direct rows and explicit quality references are documented in `references/provider-model-status.md`.

GPT-6 Astra requires live catalog verification for every selected OpenAI account and is therefore not asserted by a public example. Current OpenClaw removed Qwen Portal. Kimi Coding membership uses separate credentials from the excluded Moonshot API.
